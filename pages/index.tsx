...
const FloatingElements: React.FC = () => {
  return (
    <>
      ...
      <div className="absolute w-[500px] h-[500px] bg-slate-700 rounded-full opacity-30 blur-3xl top-[-100px] left-[-100px] z-0"></div>
      <div className="absolute w-[300px] h-[300px] bg-slate-600 rounded-full opacity-20 blur-3xl bottom-[-50px] right-[-50px] z-0"></div>
...
<div className="absolute w-[300px] h-[300px] bg-slate-600 rounded-full opacity-20 blur-3xl bottom-[-50px] right-[-50px] z-0"></div>
      ...
      <nav className="flex items-center justify-between px-8 py-6 border-b border-gray-700/50 backdrop-blur-sm z-10 relative">
        <div className="flex items-center space-x-2">
          <h1 className="text-xl font-extrabold">
            <span className="bg-gradient-to-r from-slate-400 via-white-400 to-slate-300 bg-clip-text text-transparent">
              Juster
            </span>
          </h1>
        </div>
        <ul className="flex space-x-6 text-sm font-bold">
          <li>
            <Link href="/" className="hover:text-slate-300 transition-colors duration-300 relative group">
              Home
              <span className="absolute -bottom-1 left-0 w-0 h-0.5 bg-slate-400 transition-all duration-300 group-hover:w-full"></span>
            </Link>
          </li>
          <li>
            <Link href="/skills" className="hover:text-slate-300 transition-colors duration-300 relative group">
              Skills
              <span className="absolute -bottom-1 left-0 w-0 h-0.5 bg-slate-400 transition-all duration-300 group-hover:w-full"></span>
            </Link>
          </li>
          <li>
            <Link href="/works" className="hover:text-slate-300 transition-colors duration-300 relative group">
              Works
              <span className="absolute -bottom-1 left-0 w-0 h-0.5 bg-slate-400 transition-all duration-300 group-hover:w-full"></span>
            </Link>
          </li>
          <li>
            <Link href="/contact" className="hover:text-slate-300 transition-colors duration-300 relative group">
              Contact
              <span className="absolute -bottom-1 left-0 w-0 h-0.5 bg-slate-400 transition-all duration-300 group-hover:w-full"></span>
            </Link>
          </li>
      ...
      <main className="flex flex-col md:flex-row items-center justify-center gap-12 px-4 py-10 md:py-12 xl:py-8 min-h-[70vh] xl:min-h-[75vh] z-10 relative">
        <div className="md:w-[40%] w-full mb-10 md:mb-0 text-center md:text-left flex flex-col justify-center">
          ...
          <div className="flex flex-col md:flex-row gap-4 mt-6">
            <Link href="/contact">
              <button className="px-8 py-3 text-lg bg-white text-black hover:bg-slate-500 hover:text-white transition-all duration-300 rounded-full font-semibold shadow-lg transform hover:scale-105 hover:shadow-xl">
                Get in Touch!
              </button>
            </Link>
            <Link href="/works">
              <button className="px-8 py-3 text-lg bg-slate-600/80 backdrop-blur-sm hover:bg-white hover:text-black transition-all duration-300 rounded-full font-semibold shadow-lg border border-purple-400/30 transform hover:scale-105 hover:shadow-xl">
                Browse Projects
              </button>
            </Link>
          </div>
      ...
      <footer className="flex justify-center space-x-8 py-6 mt-auto mb-8 text-gray-400 font-bold text-sm relative z-10">
        <Link href="https://linkedin.com" target="_blank" className="flex items-center space-x-2 hover:text-slate-300 transition-all duration-300 transform hover:scale-110">
          <FaLinkedin size={20} />
          <span>LinkedIn</span>
        </Link>
        <Link href="https://github.com/Strixyl" target="_blank" className="flex items-center space-x-2 hover:text-slate-300 transition-all duration-300 transform hover:scale-110">
          <FaGithub size={20} />
          <span>GitHub</span>
        </Link>
        <Link href="mailto:uretajuster@gmail.com" target="_blank" className="flex items-center space-x-2 hover:text-slate-300 transition-all duration-300 transform hover:scale-110">
          <SiGmail size={20} />
          <span>G-Mail</span>
        </Link>
      </footer>
